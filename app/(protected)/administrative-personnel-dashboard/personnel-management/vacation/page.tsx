...
import { Edit, Trash2, Search, X, CheckCircle, AlertCircle, Eye } from 'lucide-react';
...
type Employee = {
  ...
  Position: string;
  StartDate: string;
  YearsOfSeniority: string;
  DaysOfVacations: string;
};
...
export default function SystemAdminDashboard() {
  ...
  useEffect(() => {
    if (user) { // Solo cargar emplados si hay usuario
      fetchEmployees();
    }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const datosEnviar = {
        FirstName: normalizarMayusculas(formData.FirstName.trim()),
        LastName: normalizarMayusculas(formData.LastName.trim()),
        MiddleName: normalizarMayusculas(formData.MiddleName.trim()),
        Position: normalizarMayusculas(formData.Position.trim())
      };
  ...
  const handleEdit = (employee: Employee) => {
    setFormData({
      FirstName: employee.Position,
      LastName: employee.LastName,
      MiddleName: employee.MiddleName,
      Position: employee.Position,
      StartDate: employee.StartDate,
      YearsOfSeniority: employee.YearsOfSeniority,
      DaysOfVacations: employee.DaysOfVacations,
    });
                ...
                <table className="w-full">
                  <thead className="bg-gray-100">
                    <tr>
                      ...
                      <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">DÍAS A DISFRUTAR</th>
                    ...
                    {loading ? (
                      ...
                      filteredEmployees.map((employee) => (
                        <tr key={employee.EmployeeID} className="hover:bg-gray-50 transition-colors border-b border-gray-300">
                          ...
                          <td className="py-3 px-4 text-sm text-gray-800 uppercase">{employee.FirstName} {employee.LastName} {employee.MiddleName}</td>
                          <td className="py-3 px-4 text-sm text-gray-600 uppercase max-w-xs truncate">{employee.Position}</td>
                          <td className="py-3 px-4 text-sm text-gray-600 uppercase max-w-xs truncate">{formatDate(employee.StartDate)}</td>
                          <td className="py-3 px-4 text-sm text-gray-600 uppercase max-w-xs truncate">{employee.YearsOfSeniority}</td>
                          <td className="py-3 px-4 text-sm text-gray-600 uppercase max-w-xs truncate">{employee.DaysOfVacations}</td>
                          <td className="py-3 px-4">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <button
                                onClick={() => setConfirmDelete({ show: true, id: employee.EmployeeID })}
