...
// Interface para incidencia de empleado
interface EmployeeIncidence {
  IncidenceID: number;
  EmployeeID: number;
...
// Interface para formulario de incidencia
...
// Interface para detalles de éxito
...
export default function EmployeeIncidencePage() {
  ...
  // Estados
  const [records, setRecords] = useState<EmployeeIncidence[]>([]);
  const [filteredRecords, setFilteredRecords] = useState<EmployeeIncidence[]>([]);
  ...
  const [recordToEdit, setRecordToEdit] = useState<EmployeeIncidence | null>(null);
  ...
  // Función para obtener registros de incidencias
  ...
  const handleEditRecord = async (record: EmployeeIncidence) => {
    setModalMode('edit');
    setRecordToEdit(record);
    
    try {
      const response = await fetch(`/api/administrative-personnel-dashboard/employee-management/employeeincidence/search?term=${record.EmployeeID}`);
      if (response.ok) {
        const data = await response.json();
    ...
    setFormData({
      ...
      IncidenceDate: formatDateForInput(record.IncidenceDate),
      Rule: record.Rule || ''
    });
  ...
  const handleDeleteRecord = async (id: number) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setSuccessMessage('¡INCIDENCIA ELIMINADA EXITOSAMENTE!');
        await fetchRecords();
  ...
  const handleFormChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // No normalizar a mayúsculas para campos numéricos
    if (name === 'InicidenceNumber') {
  ...
  // Función para guardar registro de incidencia
  const handleSaveRecord = async () => {
    try {
      ...
      if (!formData.Rule) {
        setError('LA REGLA ES REQUERIDA');
        setSaving(false);
        return;
      }
      ...
      if (modalMode === 'create') {
        ...
        response = await fetch(`/api/administrative-personnel-dashboard/employee-management/employeeincidence/${recordToEdit?.IncidenceID}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
      ...
      if (response.ok && data.success) {
        ...
        if (modalMode === 'create' && selectedEmployeeData && data.fileUrl) {
          ...
          const excelUrl = `${baseUrl}/api/download/edit/FT-RH-27?incidenceId=${data.incidenceId}`;
          ...
          setSuccessMessage('INCIDENCIA ACTUALIZADA EXITOSAMENTE!');
          setTimeout(() => setSuccessMessage(''), 3000);
        } else {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full animate-fade-in relative z-[10000]">
            <div className="p-6 pb-4 border-b border-gray-300">
              ...
              <p className="text-sm text-gray-600 mt-2 leading-5">
                ¿Está seguro que desea eliminar esta incidencia? Esta acción no se puede deshacer.
              </p>
            </div>
      ...
      {/* MODAL DE ÉXITO CON DESCARGA DE EXCEL */}
          ...
          <div className="w-full md:w-1/3 p-6 border-r border-gray-200 overflow-y-auto">
              <div className="space-y-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  <h3 className="font-bold text-gray-800 mb-3 text-sm uppercase">DETALLES DE LA INCIDENCIA</h3>
                  <div className="space-y-3">
                     <div className="flex justify-between items-center">
                        <span className="block text-xs font-bold text-gray-700 uppercase">ID PERMISO:</span>
                        <span className="text-gray-600 mt-1 text-sm">{successDetails.IncidenceID}</span>
                      </div>
                      <div className="flex justify-between items-center">
                    ...
                    {/* PDF */}
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => handleDownloadPDF(successDetails.pdfUrl, `FT-RH-27-${successDetails.tipo}-${successDetails.EmployeeID}.pdf`)}
                     ...
                     {/* Excel Editable */}
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() => handleDownloadExcel(successDetails.excelUrl, `FT-RH-27-${successDetails.tipo}-${successDetails.EmployeeID}.xlsx`)}
                          disabled={downloading}
                          className="p-2 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {/* MODAL DE REGISTRO DE INCIDENCIA */}
      {showModal && (
        <div 
          className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70"
          style={{ margin: 0, top: 0, left: 0, right: 0, bottom: 0 }}
        >
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
            {/* Encabezado */}
                    ...
                    <div className="md:col-span-2">
                      ...
                      <div className="relative">
                        <textarea
                          ...
                          rows={3}
                          className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                          placeholder="Ingrese la regla aplicada"
                          required
                        />
                      </div>
                    </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Buscar por ID, nombre de empleado, descripción o número de incidencia..."
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">ID INCIDENCIA</th>
                    ...
                    <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">DESCRIPCIÓN</th>
                    <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">REGLA</th>
                    <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">DOCUMENTO</th>
                ...
                <tbody>
                  {loading ? (
                    ...
                    currentRecords.map((record) => {
                      ...
                      return(
                      <tr key={record.IncidenceID} className="hover:bg-gray-50 transition-colors border-b border-gray-300">
                        <td className="py-3 px-4 text-sm text-gray-800 font-medium">{record.IncidenceID}</td>
                        ...
                        <td className="py-3 px-4 text-sm text-gray-800">
                            {formatDate(record.IncidenceDate)}
                          </td>
                        <td className="py-3 px-4">
                          <div className="text-sm text-gray-800 uppercase max-w-xs truncate">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button
                              onClick={() => setConfirmDelete({ show: true, id: record.IncidenceID })}
