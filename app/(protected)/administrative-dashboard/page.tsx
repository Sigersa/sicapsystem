...
import AppHeader from '@/components/header/5/5'
import Footer from '@/components/footer'
import { useRouter } from 'next/navigation'
import { useSessionManager } from '@/hooks/useSessionManager/5'
import { useInactivityManager } from '@/hooks/useInactivityManager'
import { UserPlus, FileText, ToolCase } from 'lucide-react'
...
export default function Page() {
  ...
  const menuItems: MenuItem[] = [
    {
      ...
      href: '/administrative-dashboard/reports'
    },
    {
      id: 2,
      title: 'COMPROBANTES DE GASTOS',
      description: 'Generación de comprobantes de gastos',
      icon: <FileText className="h-6 w-6" />,
      href: '/administrative-dashboard/expenses-verification'
    },
    {
      id: 3,
      title: 'CONSULTA DE GASTOS',
      description: 'Consulta de gastos registrados en el sistema',
      icon: <ToolCase className="h-6 w-6" />,
      href: '/administrative-dashboard/expenses-inquiry'
    }
  ]
  ...
  useEffect(() => {
    ...
    const fetchProjectStatus = async () => {
      try {
        const res = await fetch('/api/administrative-dashboard/projects/status', {
          credentials: 'include'
        })
