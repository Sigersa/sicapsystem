...
interface RegistroInfraestructura {
  id: number;
  fecha: string;
  concepto: string;
  metodoPago: string;
...
interface DeleteModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  registro: RegistroInfraestructura | null;
  isDeleting: boolean;
}
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroInfraestructura | null;
  onSave: (data: RegistroInfraestructura) => void;
...
const EditModal: React.FC<EditModalProps> = ({
  ...
  const [editData, setEditData] = useState<RegistroInfraestructura | null>(null);
  ...
  const { startUpload } = useUploadThing('infrastructureFiles', {
    onClientUploadComplete: (files) => {
      console.log("Archivos subidos a infraestructura:", files);
    },
    onUploadError: (error) => {
      setModal({
        isOpen: true,
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">EDITAR REGISTRO DE INFRAESTRUCTURA</h2>
...
// ============ MODAL DE AGREGAR INFRAESTRUCTURA ============
...
const AddInfrastructureModal: React.FC<AddInfrastructureModalProps> = ({
  isOpen,
  onClose,
  onSave,
  isSubmitting,
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = infraData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
    ...
    <>
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">NUEVO REGISTRO DE INFRAESTRUCTURA</h2>
              <p className="text-gray-600 mt-1 text-sm">Complete la información del servicio de infraestructura en sitio.</p>
            </div>
            <button
              onClick={handleClose}
              className="p-2 hover:bg-gray-100 rounded-full transition-colors"
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="fecha"
                      value={infraData.fecha ? infraData.fecha.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="total"
                      value={infraData.formattedTotal || ''}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                ...
                {infraData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {infraData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
...
export default function InfrastructurePage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroInfraestructura[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroInfraestructura | null,
    isDeleting: false
  });
  ...
  const [addInfrastructureModal, setAddInfrastructureModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroInfraestructura[]) => {
    const grouped: { [key: string]: RegistroInfraestructura[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/infrastructure?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();

        setRegistros(data.map((registro: any) => ({
          id: registro.InfrastructureServiceID,
          fecha: registro.Date,
  ...
  const openDeleteModal = (registro: RegistroInfraestructura) => {
  ...
  const openEditModal = (registro: RegistroInfraestructura) => {
  ...
  const openAddInfrastructureModal = () => {
    setAddInfrastructureModal({ isOpen: true });
  };

  const closeAddInfrastructureModal = () => {
    setAddInfrastructureModal({ isOpen: false });
  };

  const handleAddInfrastructure = async (data: InfrastructureData) => {
    if (!projectId) {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/infrastructure', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newInfra = await response.json();
        const infraId = newInfra.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);

          await fetch(`/api/project-manager/projects/infrastructure?id=${infraId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              date: fechaFormateada,
        ...
        await sendNotification(infraId);
        await fetchRegistros();

        showModal('Éxito', '¡REGISTRO DE INFRAESTRUCTURA GUARDADO EXITOSAMENTE!', 'success');
  ...
  const sendNotification = async (infraId: number, isUpdate: boolean = false) => {
    try {
      await fetch('/api/notifications/notifications-infrastructure', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ projectId, infraId, isUpdate })
      });
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/infrastructure?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroInfraestructura) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/infrastructure?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <AddInfrastructureModal
        isOpen={addInfrastructureModal.isOpen}
        onClose={closeAddInfrastructureModal}
        onSave={handleAddInfrastructure}
      ...
      <main className="pt-[72px] pb-[80px] min-h-screen bg-gray-100">
        <div className="w-full px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <div className="bg-[#3a6ea5] p-4 rounded-lg shadow border border-[#3a6ea5]">
              <h1 className="text-xl font-bold text-white tracking-tight flex items-center">
                ...
                GESTIÓN DE INFRAESTRUCTURA EN SITIO
              </h1>
              <p className="text-sm text-gray-200 mt-1">
                Administre y visualice todos los registros de servicios de infraestructura en sitio del proyecto.
              </p>
              {projectId && (
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button
              onClick={openAddInfrastructureModal}
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm
                      ...
                      : 'No se han encontrado registros de servicios de infraestructura.'}
                  </p>
                </div>
              )}
            </div>
