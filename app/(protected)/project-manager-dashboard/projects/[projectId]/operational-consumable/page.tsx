...
interface OperativeConsumableData {
  fecha: Date | null;
  concepto: string;
  metodoPago: string;
  total: number;
...
interface RegistroOperativeConsumable {
  id: number;
  fecha: string;
  concepto: string;
  metodoPago: string;
...
interface DeleteModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  registro: RegistroOperativeConsumable | null;
  isDeleting: boolean;
}
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroOperativeConsumable | null;
  onSave: (data: RegistroOperativeConsumable) => void;
...
const EditModal: React.FC<EditModalProps> = ({
  ...
  const [editData, setEditData] = useState<RegistroOperativeConsumable | null>(null);
  ...
  const { startUpload } = useUploadThing('operativeConsumableFiles', {
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">EDITAR REGISTRO DE CONSUMIBLE OPERATIVO</h2>
...
// ============ MODAL DE AGREGAR CONSUMIBLE OPERATIVO ============

interface AddOperativeConsumableModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: OperativeConsumableData) => void;
  isSubmitting: boolean;
  paymentMethods: PaymentMethod[];
}

const AddOperativeConsumableModal: React.FC<AddOperativeConsumableModalProps> = ({
  isOpen,
  onClose,
  onSave,
  isSubmitting,
  ...
  const [operativeConsumableData, setOperativeConsumableData] = useState<OperativeConsumableData>({
    fecha: null,
    concepto: '',
    metodoPago: '',
    total: 0,
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'total') {
      const formattedValue = formatCurrency(value);
      setOperativeConsumableData(prev => ({
        ...prev,
        [name]: parseCurrency(formattedValue),
        formattedTotal: formattedValue
      }));
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = operativeConsumableData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    await onSave(operativeConsumableData);
  };
    ...
    <>
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">NUEVO REGISTRO DE CONSUMIBLE OPERATIVO</h2>
              <p className="text-gray-600 mt-1 text-sm">Complete la información del consumible operativo.</p>
            </div>
            <button
              onClick={handleClose}
              className="p-2 hover:bg-gray-100 rounded-full transition-colors"
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="fecha"
                      value={operativeConsumableData.fecha ? operativeConsumableData.fecha.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="concepto"
                      value={operativeConsumableData.concepto}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      name="metodoPago"
                      value={operativeConsumableData.metodoPago}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="total"
                      value={operativeConsumableData.formattedTotal || ''}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                ...
                {operativeConsumableData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {operativeConsumableData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <textarea
                  name="observaciones"
                  value={operativeConsumableData.observaciones}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium resize-none"
                  placeholder="INGRESE LAS OBSERVACIONES"
...
export default function OperativeConsumablePage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroOperativeConsumable[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroOperativeConsumable | null,
    isDeleting: false
  });
  ...
  const [addOperativeConsumableModal, setAddOperativeConsumableModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroOperativeConsumable[]) => {
    const grouped: { [key: string]: RegistroOperativeConsumable[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/operative-consumable?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();

        setRegistros(data.map((registro: any) => ({
          id: registro.OperativeConsumableID,
          fecha: registro.Date,
  ...
  const openDeleteModal = (registro: RegistroOperativeConsumable) => {
  ...
  const openEditModal = (registro: RegistroOperativeConsumable) => {
  ...
  const openAddOperativeConsumableModal = () => {
    setAddOperativeConsumableModal({ isOpen: true });
  };

  const closeAddOperativeConsumableModal = () => {
    setAddOperativeConsumableModal({ isOpen: false });
  };

  const handleAddOperativeConsumable = async (data: OperativeConsumableData) => {
    if (!projectId) {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/operative-consumable', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newOperativeConsumable = await response.json();
        const operativeConsumableId = newOperativeConsumable.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);

          await fetch(`/api/project-manager/projects/operative-consumable?id=${operativeConsumableId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              date: fechaFormateada,
        ...
        await sendNotification(operativeConsumableId);
        await fetchRegistros();

        showModal('Éxito', '¡REGISTRO DE CONSUMIBLE OPERATIVO GUARDADO EXITOSAMENTE!', 'success');
        closeAddOperativeConsumableModal();
      } else {
  ...
  const sendNotification = async (operativeConsumableId: number, isUpdate: boolean = false) => {
    try {
      await fetch('/api/notifications/notifications-operative-consumable', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ projectId, operativeConsumableId, isUpdate })
      });
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/operative-consumable?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroOperativeConsumable) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/operative-consumable?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar consumible operativo */}
      <AddOperativeConsumableModal
        isOpen={addOperativeConsumableModal.isOpen}
        onClose={closeAddOperativeConsumableModal}
        onSave={handleAddOperativeConsumable}
      ...
      <main className="pt-[72px] pb-[80px] min-h-screen bg-gray-100">
        <div className="w-full px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <div className="bg-[#3a6ea5] p-4 rounded-lg shadow border border-[#3a6ea5]">
              <h1 className="text-xl font-bold text-white tracking-tight flex items-center">
                ...
                GESTIÓN DE CONSUMIBLES OPERATIVOS
              </h1>
              <p className="text-sm text-gray-200 mt-1">
                Administre y visualice todos los registros de consumibles operativos del proyecto.
              </p>
              {projectId && (
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button
              onClick={openAddOperativeConsumableModal}
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm
                      ...
                      : 'No se han encontrado registros de consumibles operativos.'}
                  </p>
                </div>
              )}
            </div>
