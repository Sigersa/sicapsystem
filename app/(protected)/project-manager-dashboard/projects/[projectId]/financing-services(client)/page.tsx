...
interface FinancingServiceData {
  fecha: Date | null;
  concepto: string;
  metodoPago: string;
  total: number;
...
interface RegistroFinancingService {
  id: number;
  fecha: string;
  concepto: string;
  metodoPago: string;
...
interface DeleteModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  registro: RegistroFinancingService | null;
  isDeleting: boolean;
}
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroFinancingService | null;
  onSave: (data: RegistroFinancingService) => void;
...
const EditModal: React.FC<EditModalProps> = ({
  ...
  const [editData, setEditData] = useState<RegistroFinancingService | null>(null);
  ...
  const { startUpload } = useUploadThing('financingServiceFiles', {
...
// ============ MODAL DE AGREGAR SERVICIO DE FINANCIAMIENTO ============

interface AddFinancingServiceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: FinancingServiceData) => void;
  isSubmitting: boolean;
  paymentMethods: PaymentMethod[];
}

const AddFinancingServiceModal: React.FC<AddFinancingServiceModalProps> = ({
  isOpen,
  onClose,
  onSave,
  isSubmitting,
  ...
  const [financingServiceData, setFinancingServiceData] = useState<FinancingServiceData>({
    fecha: null,
    concepto: '',
    metodoPago: '',
    total: 0,
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = financingServiceData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    await onSave(financingServiceData);
  };
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="fecha"
                      value={financingServiceData.fecha ? financingServiceData.fecha.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="concepto"
                      value={financingServiceData.concepto}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      name="metodoPago"
                      value={financingServiceData.metodoPago}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="total"
                      value={financingServiceData.formattedTotal || ''}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                ...
                {financingServiceData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {financingServiceData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <textarea
                  name="observaciones"
                  value={financingServiceData.observaciones}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium resize-none"
                  placeholder="INGRESE LAS OBSERVACIONES"
...
export default function FinancingServicesPage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroFinancingService[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroFinancingService | null,
    isDeleting: false
  });
  ...
  const [addFinancingServiceModal, setAddFinancingServiceModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroFinancingService[]) => {
    const grouped: { [key: string]: RegistroFinancingService[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/financing-services?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();

        setRegistros(data.map((registro: any) => ({
          id: registro.FinancingServicesID,
          fecha: registro.Date,
  ...
  const openDeleteModal = (registro: RegistroFinancingService) => {
  ...
  const openEditModal = (registro: RegistroFinancingService) => {
  ...
  const openAddFinancingServiceModal = () => {
    setAddFinancingServiceModal({ isOpen: true });
  };

  const closeAddFinancingServiceModal = () => {
    setAddFinancingServiceModal({ isOpen: false });
  };

  const handleAddFinancingService = async (data: FinancingServiceData) => {
    if (!projectId) {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/financing-services', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newFinancingService = await response.json();
        const financingServiceId = newFinancingService.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);

          await fetch(`/api/project-manager/projects/financing-services?id=${financingServiceId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              date: fechaFormateada,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/financing-services?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroFinancingService) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/financing-services?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar servicio de financiamiento */}
      <AddFinancingServiceModal
        isOpen={addFinancingServiceModal.isOpen}
        onClose={closeAddFinancingServiceModal}
        onSave={handleAddFinancingService}
      ...
      <main className="pt-[72px] pb-[80px] min-h-screen bg-gray-100">
        <div className="w-full px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button
              onClick={openAddFinancingServiceModal}
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm
                      ...
                      : 'No se han encontrado registros de servicios de financiamiento.'}
                  </p>
                </div>
              )}
            </div>
