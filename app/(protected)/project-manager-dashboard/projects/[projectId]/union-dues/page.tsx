...
interface DeleteModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  registro: RegistroUnionDue | null;
  isDeleting: boolean;
}
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full animate-fade-in relative z-[10000]">
        <div className="p-6 pb-4 border-b border-gray-300">
          ...
          <p className="text-sm text-gray-600 mt-2 leading-5">
            ¿Está seguro que desea eliminar el registro <span className="font-bold">{registro?.concept}</span>?
          </p>
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroUnionDue | null;
  onSave: (data: RegistroUnionDue) => void;
...
const EditModal: React.FC<EditModalProps> = ({
  ...
  const [editData, setEditData] = useState<RegistroUnionDue | null>(null);
  ...
  const { startUpload } = useUploadThing('unionDueFiles', {
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    } else if (name === 'concept' || name === 'observations') {
      setEditData({
        ...editData,
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">EDITAR REGISTRO DE CUOTA SINDICAL</h2>
...
// ============ MODAL DE AGREGAR CUOTA SINDICAL ============

interface AddUnionDueModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: UnionDueData) => void;
  isSubmitting: boolean;
  paymentMethods: PaymentMethod[];
}

const AddUnionDueModal: React.FC<AddUnionDueModalProps> = ({
  isOpen,
  onClose,
  onSave,
  isSubmitting,
  ...
  const [unionDueData, setUnionDueData] = useState<UnionDueData>({
    date: null,
    concept: '',
    startDate: null,
    endDate: null,
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = unionDueData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
    ...
    <>
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">NUEVO REGISTRO DE CUOTA SINDICAL</h2>
              <p className="text-gray-600 mt-1 text-sm">Complete la información de la cuota sindical.</p>
            </div>
            <button
              onClick={handleClose}
              className="p-2 hover:bg-gray-100 rounded-full transition-colors"
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="date"
                      value={unionDueData.date ? unionDueData.date.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="total"
                      value={unionDueData.formattedTotal || ''}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="startDate"
                      value={unionDueData.startDate ? unionDueData.startDate.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
                        const date = e.target.value ? new Date(e.target.value) : null;
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      name="endDate"
                      value={unionDueData.endDate ? unionDueData.endDate.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
                        const date = e.target.value ? new Date(e.target.value) : null;
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                {unionDueData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {unionDueData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <textarea
                  name="observations"
                  value={unionDueData.observations}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium resize-none"
                  placeholder="INGRESE LAS OBSERVACIONES"
...
export default function UnionDuesPage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroUnionDue[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroUnionDue | null,
    isDeleting: false
  });
  ...
  const [addUnionDueModal, setAddUnionDueModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroUnionDue[]) => {
    const grouped: { [key: string]: RegistroUnionDue[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/union-dues?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const openDeleteModal = (registro: RegistroUnionDue) => {
  ...
  const openEditModal = (registro: RegistroUnionDue) => {
  ...
  const openAddUnionDueModal = () => {
    setAddUnionDueModal({ isOpen: true });
  };

  const closeAddUnionDueModal = () => {
    setAddUnionDueModal({ isOpen: false });
  };

  const handleAddUnionDue = async (data: UnionDueData) => {
    if (!projectId) {
      showModal('Error', 'No se ha identificado el proyecto asociado', 'error');
      return;
    }

    if (!data.date || !data.concept) {
      showModal('Error', 'La fecha y el concepto son requeridos', 'error');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/union-dues', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newUnionDue = await response.json();
        const unionDueId = newUnionDue.id;

        await sendNotification(unionDueId);
        await fetchRegistros();

        showModal('Éxito', '¡REGISTRO DE CUOTA SINDICAL GUARDADO EXITOSAMENTE!', 'success');
        closeAddUnionDueModal();
      } else {
  ...
  const sendNotification = async (unionDueId: number, isUpdate: boolean = false) => {
    try {
      await fetch('/api/notifications/notifications-union-dues', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ projectId, unionDueId, isUpdate })
      });
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/union-dues?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroUnionDue) => {
    ...
    try {
      const fechaFormateada = data.date.includes('T')
        ? data.date.split('T')[0]
        : data.date;
      const startDateFormatted = data.startDate && data.startDate.includes('T')
        ? data.startDate.split('T')[0]
        : data.startDate;
      const endDateFormatted = data.endDate && data.endDate.includes('T')
        ? data.endDate.split('T')[0]
        : data.endDate;

      const response = await fetch(`/api/project-manager/projects/union-dues?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  const filteredRegistros = registros.filter(registro => {
    const concept = (registro.concept || '').toLowerCase();
    const observations = (registro.observations || '').toLowerCase();
    const methodName = (registro.methodName || '').toLowerCase();
    const term = searchTerm.toLowerCase();

    return (
      concept.includes(term) ||
      observations.includes(term) ||
      methodName.includes(term)
    );
  });
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <AppHeader title="GESTIÓN DE CUOTAS SINDICALES" />
      ...
      {/* Modal de agregar cuota sindical */}
      <AddUnionDueModal
        isOpen={addUnionDueModal.isOpen}
        onClose={closeAddUnionDueModal}
        onSave={handleAddUnionDue}
          ...
          <div className="mb-6">
            <div className="bg-[#3a6ea5] p-4 rounded-lg shadow border border-[#3a6ea5]">
              <h1 className="text-xl font-bold text-white tracking-tight flex items-center">
                ...
                GESTIÓN DE CUOTAS SINDICALES
              </h1>
              <p className="text-sm text-gray-200 mt-1">
                Administre y visualice todos los registros de cuotas sindicales del proyecto.
              </p>
              {projectId && (
                            ...
                            return (
                              <tr key={registro.id} className="hover:bg-gray-50 transition-colors border-b border-gray-300">
                                <td className="py-3 px-4">
                                  <div className="text-sm text-gray-800">{formatDate(registro.date)}</div>
                                </td>
                                <td className="py-3 px-4">
                                  <div className="text-sm font-medium text-gray-800 uppercase max-w-[150px] truncate" title={registro.concept}>
                                    {registro.concept}
                                  </div>
                                </td>
                                <td className="py-3 px-4">
                                  <div className="text-sm text-gray-800">
                                    {registro.startDate ? formatDate(registro.startDate) : '-'}
                                  </div>
                                </td>
                                <td className="py-3 px-4">
                                  <div className="text-sm text-gray-800">
                                    {registro.endDate ? formatDate(registro.endDate) : '-'}
                                  </div>
                                </td>
                                <td className="py-3 px-4">
                                ...
                                <td className="py-3 px-4">
                                  <div className="text-sm text-gray-800 max-w-[180px] truncate" title={registro.observations}>
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm
                      ...
                      : 'No se han encontrado registros de cuotas sindicales.'}
                  </p>
                </div>
              )}
            </div>
