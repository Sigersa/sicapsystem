...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroNomina | null;
  onSave: (data: RegistroNomina) => void;
...
const EditModal: React.FC<EditModalProps> = ({ 
  ...
  const [editData, setEditData] = useState<RegistroNomina | null>(null);
  ...
  const { startUpload } = useUploadThing('payrollFiles', {
    onClientUploadComplete: (files) => {
      console.log("Archivos subidos a nómina:", files);
    },
    onUploadError: (error) => {
      setModal({
        isOpen: true,
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              ...
              <p className="text-gray-600 mt-1 text-sm">Modifique la información del registro de nómina.</p>
            </div>
            <button
              onClick={handleClose}
...
// ============ MODAL DE AGREGAR NÓMINA ============

interface AddNominaModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: NominaData) => void;
  isSubmitting: boolean;
  paymentMethods: PaymentMethod[];
}

const AddNominaModal: React.FC<AddNominaModalProps> = ({ 
  isOpen, 
  onClose, 
  onSave, 
  isSubmitting, 
  ...
  const [nominaData, setNominaData] = useState<NominaData>({
    fecha: null,
    concepto: '',
    metodoPago: '',
    total: 0,
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = nominaData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="fecha"
                      value={nominaData.fecha ? nominaData.fecha.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                {nominaData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {nominaData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <textarea
                  name="observaciones"
                  value={nominaData.observaciones}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium resize-none"
                  placeholder="INGRESE LAS OBSERVACIONES"
...
export default function PayrollPage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroNomina[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroNomina | null,
    isDeleting: false
  });
  ...
  const [addNominaModal, setAddNominaModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroNomina[]) => {
    const grouped: { [key: string]: RegistroNomina[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/payroll?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const openDeleteModal = (registro: RegistroNomina) => {
  ...
  const openEditModal = (registro: RegistroNomina) => {
  ...
  const openAddNominaModal = () => {
    setAddNominaModal({ isOpen: true });
  };

  const closeAddNominaModal = () => {
    setAddNominaModal({ isOpen: false });
  };

  const handleAddNomina = async (data: NominaData) => {
    if (!projectId) {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/payroll', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newNomina = await response.json();
        const nominaId = newNomina.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);
          
          await fetch(`/api/project-manager/projects/payroll?id=${nominaId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              date: fechaFormateada,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/payroll?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroNomina) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/payroll?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar nómina */}
      <AddNominaModal
        isOpen={addNominaModal.isOpen}
        onClose={closeAddNominaModal}
        onSave={handleAddNomina}
