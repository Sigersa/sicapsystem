...
interface RegistroToolsEquipment {
  id: number;
  fecha: string;
  concepto: string;
  metodoPago: string;
...
interface DeleteModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  registro: RegistroToolsEquipment | null;
  isDeleting: boolean;
}
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroToolsEquipment | null;
  onSave: (data: RegistroToolsEquipment) => void;
...
const EditModal: React.FC<EditModalProps> = ({ 
  ...
  const [editData, setEditData] = useState<RegistroToolsEquipment | null>(null);
  ...
  const { startUpload } = useUploadThing('toolsEquipmentFiles', {
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              ...
              <p className="text-gray-600 mt-1 text-sm">Modifique la información del registro de herramientas y/o equipos.</p>
            </div>
            <button
              onClick={handleClose}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="HERRAMIENTA MENOR">HERRAMIENTA MENOR</option>
...
// ============ MODAL DE AGREGAR HERRAMIENTAS/EQUIPOS ============

interface AddToolsEquipmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: ToolsEquipmentData) => void;
  isSubmitting: boolean;
  paymentMethods: PaymentMethod[];
}

const AddToolsEquipmentModal: React.FC<AddToolsEquipmentModalProps> = ({ 
  isOpen, 
  onClose, 
  onSave, 
  isSubmitting, 
  ...
  const [toolsEquipmentData, setToolsEquipmentData] = useState<ToolsEquipmentData>({
    fecha: null,
    concepto: '',
    metodoPago: '',
    total: 0,
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = toolsEquipmentData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    await onSave(toolsEquipmentData);
  };
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="fecha"
                      value={toolsEquipmentData.fecha ? toolsEquipmentData.fecha.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
                  ...
                  <div>
                    ...
                    <select
                      name="concepto"
                      value={toolsEquipmentData.concepto}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
                  ...
                  <div>
                    ...
                    <select
                      name="metodoPago"
                      value={toolsEquipmentData.metodoPago}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                {toolsEquipmentData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {toolsEquipmentData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <textarea
                  name="observaciones"
                  value={toolsEquipmentData.observaciones}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium resize-none"
                  placeholder="INGRESE LAS OBSERVACIONES"
...
export default function ToolsEquipmentPage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroToolsEquipment[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroToolsEquipment | null,
    isDeleting: false
  });
  ...
  const [addToolsEquipmentModal, setAddToolsEquipmentModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroToolsEquipment[]) => {
    const grouped: { [key: string]: RegistroToolsEquipment[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/tools-equipment?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();
        setRegistros(data.map((item: any) => ({
          id: item.ToolsequipmentID,
          fecha: item.Date,
  ...
  const openDeleteModal = (registro: RegistroToolsEquipment) => {
  ...
  const openEditModal = (registro: RegistroToolsEquipment) => {
  ...
  const openAddToolsEquipmentModal = () => {
    setAddToolsEquipmentModal({ isOpen: true });
  };

  const closeAddToolsEquipmentModal = () => {
    setAddToolsEquipmentModal({ isOpen: false });
  };

  const handleAddToolsEquipment = async (data: ToolsEquipmentData) => {
    if (!projectId) {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/tools-equipment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newToolsEquipment = await response.json();
        const toolsEquipmentId = newToolsEquipment.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);
          
          await fetch(`/api/project-manager/projects/tools-equipment?id=${toolsEquipmentId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              date: fechaFormateada,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/tools-equipment?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroToolsEquipment) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/tools-equipment?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar herramientas/equipos */}
      <AddToolsEquipmentModal
        isOpen={addToolsEquipmentModal.isOpen}
        onClose={closeAddToolsEquipmentModal}
        onSave={handleAddToolsEquipment}
      ...
      <main className="pt-[72px] pb-[80px] min-h-screen bg-gray-100">
        <div className="w-full px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button
              onClick={openAddToolsEquipmentModal}
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm 
                      ...
                      : 'No se han encontrado registros de herramientas y/o equipos.'}
                  </p>
                </div>
              )}
            </div>
