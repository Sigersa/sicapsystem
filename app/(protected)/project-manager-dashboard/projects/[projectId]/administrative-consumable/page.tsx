...
interface AdministrativeConsumableData {
  fecha: Date | null;
  concepto: string;
  metodoPago: string;
  total: number;
...
interface RegistroAdministrativeConsumable {
  id: number;
  fecha: string;
  concepto: string;
  metodoPago: string;
...
interface DeleteModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  registro: RegistroAdministrativeConsumable | null;
  isDeleting: boolean;
}
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full animate-fade-in relative z-[10000]">
        <div className="p-6 pb-4 border-b border-gray-300">
          ...
          <p className="text-sm text-gray-600 mt-2 leading-5">
            ¿Está seguro que desea eliminar el registro de <span className="font-bold">{registro?.concepto}</span>?
          </p>
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroAdministrativeConsumable | null;
  onSave: (data: RegistroAdministrativeConsumable) => void;
...
const EditModal: React.FC<EditModalProps> = ({ 
  ...
  const [editData, setEditData] = useState<RegistroAdministrativeConsumable | null>(null);
  ...
  const { startUpload } = useUploadThing('administrativeConsumableFiles', {
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              ...
              <p className="text-gray-600 mt-1 text-sm">Modifique la información del consumible administrativo.</p>
            </div>
            <button
              onClick={handleClose}
...
// ============ MODAL DE AGREGAR CONSUMIBLE ADMINISTRATIVO ============

interface AddAdministrativeConsumableModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: AdministrativeConsumableData) => void;
  isSubmitting: boolean;
  paymentMethods: PaymentMethod[];
}

const AddAdministrativeConsumableModal: React.FC<AddAdministrativeConsumableModalProps> = ({ 
  isOpen, 
  onClose, 
  onSave, 
  isSubmitting, 
  ...
  const [administrativeConsumableData, setAdministrativeConsumableData] = useState<AdministrativeConsumableData>({
    fecha: null,
    concepto: '',
    metodoPago: '',
    total: 0,
  ...
  const handleClose = () => {
    setAdministrativeConsumableData({
      fecha: null,
      concepto: '',
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'total') {
      const formattedValue = formatCurrency(value);
      setAdministrativeConsumableData(prev => ({
        ...prev,
        [name]: parseCurrency(formattedValue),
        formattedTotal: formattedValue
      }));
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = administrativeConsumableData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    await onSave(administrativeConsumableData);
  };
    ...
    <>
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">NUEVO CONSUMIBLE ADMINISTRATIVO</h2>
              <p className="text-gray-600 mt-1 text-sm">Complete la información del consumible administrativo.</p>
            </div>
            <button
              onClick={handleClose}
              className="p-2 hover:bg-gray-100 rounded-full transition-colors"
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="fecha"
                      value={administrativeConsumableData.fecha ? administrativeConsumableData.fecha.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="concepto"
                      value={administrativeConsumableData.concepto}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                  ...
                  <div>
                    ...
                    <select
                      name="metodoPago"
                      value={administrativeConsumableData.metodoPago}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      name="total"
                      value={administrativeConsumableData.formattedTotal || ''}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                ...
                {administrativeConsumableData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {administrativeConsumableData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <textarea
                  name="observaciones"
                  value={administrativeConsumableData.observaciones}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium resize-none"
                  placeholder="INGRESE LAS OBSERVACIONES"
...
export default function AdministrativeConsumablePage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroAdministrativeConsumable[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroAdministrativeConsumable | null,
    isDeleting: false
  });
  ...
  const [addAdministrativeConsumableModal, setAddAdministrativeConsumableModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroAdministrativeConsumable[]) => {
    const grouped: { [key: string]: RegistroAdministrativeConsumable[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/administrative-consumable?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();
        setRegistros(data.map((item: any) => ({
          id: item.AdministrativeConsumableID,
          fecha: item.Date,
          concepto: item.Concept,
          metodoPago: item.MethodID?.toString() || '',
  ...
  const openDeleteModal = (registro: RegistroAdministrativeConsumable) => {
  ...
  const openEditModal = (registro: RegistroAdministrativeConsumable) => {
  ...
  const openAddAdministrativeConsumableModal = () => {
    setAddAdministrativeConsumableModal({ isOpen: true });
  };

  const closeAddAdministrativeConsumableModal = () => {
    setAddAdministrativeConsumableModal({ isOpen: false });
  };

  const handleAddAdministrativeConsumable = async (data: AdministrativeConsumableData) => {
    if (!projectId) {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/administrative-consumable', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newAdministrativeConsumable = await response.json();
        const administrativeConsumableId = newAdministrativeConsumable.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);
          
          await fetch(`/api/project-manager/projects/administrative-consumable?id=${administrativeConsumableId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              date: fechaFormateada,
        ...
        closeAddAdministrativeConsumableModal();
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/administrative-consumable?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroAdministrativeConsumable) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/administrative-consumable?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar consumible administrativo */}
      <AddAdministrativeConsumableModal
        isOpen={addAdministrativeConsumableModal.isOpen}
        onClose={closeAddAdministrativeConsumableModal}
        onSave={handleAddAdministrativeConsumable}
      ...
      <main className="pt-[72px] pb-[80px] min-h-screen bg-gray-100">
        <div className="w-full px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button
              onClick={openAddAdministrativeConsumableModal}
                            ...
                            return (
                              <tr key={registro.id} className="hover:bg-gray-50 transition-colors border-b border-gray-300">
                                ...
                                <td className="py-3 px-4">
                                  <div className="text-sm font-medium text-gray-800 uppercase max-w-[200px] truncate" title={registro.concepto}>
                                    {registro.concepto}
                                  </div>
                                </td>
                                <td className="py-3 px-4">
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm 
                      ...
                      : 'No se han encontrado registros de consumibles administrativos.'}
                  </p>
                </div>
              )}
            </div>
