...
const EditModal: React.FC<EditModalProps> = ({ 
  ...
  const { startUpload } = useUploadThing('infrastructureTransferFiles', {
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    } else if (name === 'vehicle' || name === 'startingPoint' || name === 'arrivalPoint' || name === 'observations') {
      setEditData({
        ...editData,
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              ...
              <p className="text-gray-600 mt-1 text-sm">Modifique la información del registro de traslado de infraestructura.</p>
            </div>
            <button
              onClick={handleClose}
    ...
    <>
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">NUEVO REGISTRO DE TRASLADO DE INFRAESTRUCTURA</h2>
              <p className="text-gray-600 mt-1 text-sm">Complete la información del traslado de infraestructura.</p>
            </div>
            <button
              onClick={handleClose}
              className="p-2 hover:bg-gray-100 rounded-full transition-colors"
...
export default function InfrastructureTransferPage({ params }: PageProps) {
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/infrastructure-transfer?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();
        setRegistros(data.map((item: any) => ({
          id: item.InfrastructureTransferID,
          date: item.Date,
  ...
  const handleAddTransfer = async (data: TransferData) => {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/infrastructure-transfer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        ...
        showModal('Éxito', '¡REGISTRO DE TRASLADO DE INFRAESTRUCTURA GUARDADO EXITOSAMENTE!', 'success');
  ...
  const sendNotification = async (transferId: number, isUpdate: boolean = false) => {
    try {
      await fetch('/api/notifications/notifications-infrastructure-transfer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/infrastructure-transfer?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroTransfer) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/infrastructure-transfer?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  const filteredRegistros = registros.filter(registro =>
    ...
    registro.vehicle.toLowerCase().includes(searchTerm.toLowerCase()) ||
    registro.observations.toLowerCase().includes(searchTerm.toLowerCase()) ||
    registro.methodName.toLowerCase().includes(searchTerm.toLowerCase())
  );
          ...
          <div className="mb-6">
            <div className="bg-[#3a6ea5] p-4 rounded-lg shadow border border-[#3a6ea5]">
              <h1 className="text-xl font-bold text-white tracking-tight">
                GESTIÓN DE TRASLADOS DE INFRAESTRUCTURA
              </h1>
              <p className="text-sm text-gray-200 mt-1">
                Administre y visualice todos los registros de traslados de infraestructura del proyecto.
              </p>
              {projectId && (
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Buscar por ruta, vehículo, observaciones..."
                  value={searchTerm}
                            ...
                            return (
                              <tr key={registro.id} className="hover:bg-gray-50 transition-colors border-b border-gray-300">
                                ...
                                <td className="py-3 px-4">
                                  <div className="text-sm font-medium text-gray-800 uppercase max-w-[150px] truncate" title={registro.vehicle}>
                                    {registro.vehicle}
                                  </div>
                                </td>
                                <td className="py-3 px-4">
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm 
                      ...
                      : 'No se han encontrado registros de traslados de infraestructura.'}
                  </p>
                </div>
              )}
            </div>
