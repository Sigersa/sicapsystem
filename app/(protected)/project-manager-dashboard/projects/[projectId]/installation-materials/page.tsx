...
interface InstallationMaterialsData {
  fecha: string | null;
  concepto: string;
  metodoPago: string;
  total: number;
...
interface RegistroInstallationMaterialsData {
  id: number;
  fecha: string;
  concepto: string;
  metodoPago: string;
...
interface DeleteModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  registro: RegistroInstallationMaterialsData | null;
  isDeleting: boolean;
}
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroInstallationMaterialsData | null;
  onSave: (data: RegistroInstallationMaterialsData) => void;
...
const EditModal: React.FC<EditModalProps> = ({
  ...
  const [editData, setEditData] = useState<RegistroInstallationMaterialsData | null>(null);
  ...
  const { startUpload } = useUploadThing('installationMaterialsFiles', {
...
// ============ MODAL DE AGREGAR MATERIALES DE INSTALACIÓN ============

interface AddInstallationMaterialsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: InstallationMaterialsData) => void;
  isSubmitting: boolean;
  paymentMethods: PaymentMethod[];
}

const AddInstallationMaterialsModal: React.FC<AddInstallationMaterialsModalProps> = ({
  isOpen,
  onClose,
  onSave,
  isSubmitting,
  ...
  const [installationMaterialsData, setInstallationMaterialsData] = useState<InstallationMaterialsData>({
    fecha: null,
    concepto: '',
    metodoPago: '',
    total: 0,
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'total') {
      const formattedValue = formatCurrency(value);
      setInstallationMaterialsData(prev => ({
        ...prev,
        [name]: parseCurrency(formattedValue),
        formattedTotal: formattedValue
      }));
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = installationMaterialsData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    await onSave(installationMaterialsData);
  };
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="fecha"
                      value={installationMaterialsData.fecha || ''}
                  ...
                  <div>
                    ...
                    <select
                      name="concepto"
                      value={installationMaterialsData.concepto}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
                  ...
                  <div>
                    ...
                    <select
                      name="metodoPago"
                      value={installationMaterialsData.metodoPago}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                {installationMaterialsData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {installationMaterialsData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <textarea
                  name="observaciones"
                  value={installationMaterialsData.observaciones}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium resize-none"
                  placeholder="INGRESE LAS OBSERVACIONES"
...
export default function InstallationMaterialsPage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroInstallationMaterialsData[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroInstallationMaterialsData | null,
    isDeleting: false
  });
  ...
  const [addInstallationMaterialsModal, setAddInstallationMaterialsModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroInstallationMaterialsData[]) => {
    const grouped: { [key: string]: RegistroInstallationMaterialsData[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/installation-materials?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();

        setRegistros(data.map((registro: any) => ({
          id: registro.InstallationMaterialID,
          fecha: registro.Date,
  ...
  const openDeleteModal = (registro: RegistroInstallationMaterialsData) => {
  ...
  const openEditModal = (registro: RegistroInstallationMaterialsData) => {
  ...
  const openAddInstallationMaterialsModal = () => {
    setAddInstallationMaterialsModal({ isOpen: true });
  };

  const closeAddInstallationMaterialsModal = () => {
    setAddInstallationMaterialsModal({ isOpen: false });
  };

  const handleAddInstallationMaterials = async (data: InstallationMaterialsData) => {
    if (!projectId) {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/installation-materials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newInstallationMaterials = await response.json();
        const installationMaterialsId = newInstallationMaterials.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);

          await fetch(`/api/project-manager/projects/installation-materials?id=${installationMaterialsId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              date: fechaFormateada,
        ...
        closeAddInstallationMaterialsModal();
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/installation-materials?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroInstallationMaterialsData) => {
    setEditModal(prev => ({ ...prev, isSaving: true }));

    try {
      const fechaFormateada = data.fecha;

      const response = await fetch(`/api/project-manager/projects/installation-materials?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar materiales de instalación */}
      <AddInstallationMaterialsModal
        isOpen={addInstallationMaterialsModal.isOpen}
        onClose={closeAddInstallationMaterialsModal}
        onSave={handleAddInstallationMaterials}
      ...
      <main className="pt-[72px] pb-[80px] min-h-screen bg-gray-100">
        <div className="w-full px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button
              onClick={openAddInstallationMaterialsModal}
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm
                      ...
                      : 'No se han encontrado registros de materiales de instalación.'}
                  </p>
                </div>
              )}
            </div>
