...
interface HospedajeData {
  fechaInicio: Date | null;
  fechaFin: Date | null;
  lugarAlojamiento: string;
  metodoPago: string;
  tarifa: number;
  formattedTarifa?: string;
  noches: number;
  total: number;
...
interface DeleteModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  registro: RegistroHospedaje | null;
  isDeleting: boolean;
}
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full animate-fade-in relative z-[10000]">
        <div className="p-6 pb-4 border-b border-gray-300">
          ...
          <p className="text-sm text-gray-600 mt-2 leading-5">
            ¿Está seguro que desea eliminar el registro de <span className="font-bold">{registro?.lugarAlojamiento}</span>?
          </p>
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroHospedaje | null;
  onSave: (data: RegistroHospedaje) => void;
...
const EditModal: React.FC<EditModalProps> = ({ 
  ...
  const [editData, setEditData] = useState<RegistroHospedaje | null>(null);
  ...
  const { startUpload } = useUploadThing('lodgingFiles', {
    onClientUploadComplete: (files) => {
      console.log("Archivos subidos a hospedaje:", files);
    },
    onUploadError: (error) => {
      setModal({
        isOpen: true,
  ...
  useEffect(() => {
    if (registro) {
      setEditData({
        ...registro,
        formattedTarifa: registro.tarifa !== 0 ? 
          registro.tarifa.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '',
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'tarifa') {
      ...
      const newTarifa = parseCurrency(formattedValue);
      
      setEditData({
        ...editData,
        [name]: newTarifa,
        formattedTarifa: formattedValue,
        total: newTarifa * editData.noches,
        formattedTotal: (newTarifa * editData.noches).toLocaleString('en-US', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2
        })
      });
    } else if (name === 'noches') {
      const noches = parseInt(value) || 0;
      setEditData({
        ...editData,
        [name]: noches,
        total: editData.tarifa * noches,
        formattedTotal: (editData.tarifa * noches).toLocaleString('en-US', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2
        })
      });
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              ...
              <p className="text-gray-600 mt-1 text-sm">Modifique la información del registro de hospedaje.</p>
            </div>
            <button
              onClick={handleClose}
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      name="lugarAlojamiento"
                      value={editData.lugarAlojamiento}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      name="tarifa"
                      value={editData.formattedTarifa || ''}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 text-sm bg-gray-100 border border-gray-400 rounded font-medium text-gray-600"
                      disabled
                    />
                  </div>
                </div>
...
// ============ MODAL DE AGREGAR HOSPEDAJE ============

interface AddHospedajeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: HospedajeData) => void;
  isSubmitting: boolean;
  paymentMethods: PaymentMethod[];
}

const AddHospedajeModal: React.FC<AddHospedajeModalProps> = ({ 
  isOpen, 
  onClose, 
  onSave, 
  isSubmitting, 
  ...
  const [hospedajeData, setHospedajeData] = useState<HospedajeData>({
    fechaInicio: null,
    fechaFin: null,
    lugarAlojamiento: '',
    metodoPago: '',
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'tarifa') {
      ...
      setHospedajeData(prev => ({
        ...prev,
        [name]: newTarifa,
        formattedTarifa: formattedValue,
        total: newTarifa * prev.noches,
        formattedTotal: (newTarifa * prev.noches).toLocaleString('en-US', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2
        })
      }));
      ...
      setHospedajeData(prev => ({
        ...prev,
        [name]: noches,
        total: prev.tarifa * noches,
        formattedTotal: (prev.tarifa * noches).toLocaleString('en-US', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2
        })
      }));
    } else if (name === 'lugarAlojamiento' || name === 'observaciones') {
      setHospedajeData(prev => ({
        ...prev,
        [name]: normalizarMayusculas(value)
      }));
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = hospedajeData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      name="lugarAlojamiento"
                      value={hospedajeData.lugarAlojamiento}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      name="fechaInicio"
                      value={hospedajeData.fechaInicio ? hospedajeData.fechaInicio.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
                        const date = e.target.value ? new Date(e.target.value) : null;
                        handleDateChange(date, 'fechaInicio');
                      }}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      name="tarifa"
                      value={hospedajeData.formattedTarifa || ''}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="total"
                      value={hospedajeData.formattedTotal || ''}
                      className="w-full px-3 py-2.5 text-sm bg-gray-100 border border-gray-400 rounded font-medium text-gray-600"
                      disabled
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      name="metodoPago"
                      value={hospedajeData.metodoPago}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
                ...
                {hospedajeData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {hospedajeData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <textarea
                  name="observaciones"
                  value={hospedajeData.observaciones}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium resize-none"
                  placeholder="INGRESE LAS OBSERVACIONES"
...
export default function LodgingPage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroHospedaje[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroHospedaje | null,
    isDeleting: false
  });
  ...
  const [addHospedajeModal, setAddHospedajeModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroHospedaje[]) => {
    const grouped: { [key: string]: RegistroHospedaje[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/lodging?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();
        setRegistros(data.map((item: any) => ({
          id: item.LodgingID,
          lugarAlojamiento: item.PlaceAccommodation,
          metodoPago: item.MethodID?.toString() || item.PaymentMethod,
          fechaInicio: item.CheckInDate,
          tarifa: item.Fee,
          formattedTarifa: item.Fee?.toLocaleString('en-US', { minimumFractionDigits: 2 }) || '',
          noches: item.Nights,
          total: item.Total,
  ...
  const openDeleteModal = (registro: RegistroHospedaje) => {
  ...
  const openEditModal = (registro: RegistroHospedaje) => {
  ...
  const openAddHospedajeModal = () => {
    setAddHospedajeModal({ isOpen: true });
  };

  const closeAddHospedajeModal = () => {
    setAddHospedajeModal({ isOpen: false });
  };

  const handleAddHospedaje = async (data: HospedajeData) => {
    if (!projectId) {
    ...
    if (!data.fechaInicio) {
      showModal('Error', 'La fecha de inicio es requerida', 'error');
      return;
    }

    setIsSubmitting(true);
    try {
      const fechaFormateada = data.fechaInicio.toISOString().split('T')[0];
      
      const response = await fetch('/api/project-manager/projects/lodging', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
          placeAccommodation: data.lugarAlojamiento,
          nights: data.noches,
          checkInDate: fechaFormateada,
      ...
      if (response.ok) {
        ...
        const lodgingId = newLodging.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);
          
          await fetch(`/api/project-manager/projects/lodging?id=${lodgingId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              placeAccommodation: data.lugarAlojamiento,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/lodging?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroHospedaje) => {
    setEditModal(prev => ({ ...prev, isSaving: true }));
    
    try {
      const fechaFormateada = data.fechaInicio.includes('T') 
        ? data.fechaInicio.split('T')[0] 
        : data.fechaInicio;

      const response = await fetch(`/api/project-manager/projects/lodging?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          placeAccommodation: data.lugarAlojamiento,
  ...
  const filteredRegistros = registros.filter(registro =>
    registro.lugarAlojamiento.toLowerCase().includes(searchTerm.toLowerCase()) ||
    registro.observaciones.toLowerCase().includes(searchTerm.toLowerCase()) ||
    registro.metodoPago.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar hospedaje */}
      <AddHospedajeModal
        isOpen={addHospedajeModal.isOpen}
        onClose={closeAddHospedajeModal}
        onSave={handleAddHospedaje}
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Buscar por lugar, observaciones..."
                  value={searchTerm}
                    ...
                    <div key={weekRange}>
                      ...
                      <table className="w-full">
                        <thead className="bg-gray-100">
                          <tr>
                            <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">LUGAR</th>
                            <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">PAGO</th>
                            <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">FECHAS</th>
                            <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">TARIFA</th>
                            <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">NOCHES</th>
                        ...
                        <tbody>
                          {weekRegistros.map((registro) => {
                            ...
                            fechaFin.setDate(fechaFin.getDate() + registro.noches - 1);
                            ...
                            return (
                              <tr key={registro.id} className="hover:bg-gray-50 transition-colors border-b border-gray-300">
                                <td className="py-3 px-4">
                                  <div className="text-sm font-medium text-gray-800 uppercase max-w-[150px] truncate" title={registro.lugarAlojamiento}>
                                    {registro.lugarAlojamiento}
                                  </div>
                                </td>
                                <td className="py-3 px-4">
                                ...
                                <td className="py-3 px-4">
                                  <div className="text-sm text-gray-800">
                                    <div>{formatDate(registro.fechaInicio)}</div>
                                    {registro.noches > 1 && (
                                      <div className="text-xs text-gray-500">
                                        a {fechaFin.toLocaleDateString('es-MX', { day: '2-digit', month: '2-digit', year: 'numeric' })}
                                      </div>
                                    )}
                                  </div>
                                </td>
                                <td className="py-3 px-4 text-sm text-gray-800 font-medium">
                                  {formatCurrencyNumber(registro.tarifa)}
                                </td>
                                ...
                                <td className="py-3 px-4">
                                  <div className="text-sm text-gray-800 max-w-[150px] truncate" title={registro.observaciones}>
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm 
                      ...
                      : 'No se han encontrado registros de hospedajes.'}
                  </p>
                </div>
              )}
            </div>
