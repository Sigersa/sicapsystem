...
interface OutsourcedServicesData {
  fecha: Date | null;
  concepto: string;
  metodoPago: string;
  total: number;
...
interface RegistroOutsourcedServices {
  id: number;
  fecha: string;
  concepto: string;
  metodoPago: string;
...
interface DeleteModalProps {
  ...
  onClose: () => void;
  onConfirm: () => void;
  registro: RegistroOutsourcedServices | null;
  isDeleting: boolean;
}
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroOutsourcedServices | null;
  onSave: (data: RegistroOutsourcedServices) => void;
...
const EditModal: React.FC<EditModalProps> = ({
  ...
  const [editData, setEditData] = useState<RegistroOutsourcedServices | null>(null);
  ...
  const { startUpload } = useUploadThing('outsourcedServicesFiles', {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="Pruebas no destructivas">PRUEBAS NO DESTRUCTIVAS</option>
                      <option value="Reelebado de esfuerzos">REELEBADO DE ESFUERZOS</option>
                      <option value="Técnicos certificados">TÉCNICOS CERTIFICADOS</option>
                      <option value="Procedimientos de soldadura">PROCEDIMIENTOS DE SOLDADURA</option>
                      <option value="Maquinados">MAQUINADOS</option>
                      <option value="Fabricantes">FABRICANTES</option>
                      <option value="Limpiezas de sistemas de lubricación">LIMPIEZAS DE SISTEMAS DE LUBRICACIÓN</option>
...
// ============ MODAL DE AGREGAR SERVICIO SUBCONTRATADO ============

interface AddOutsourcedServiceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: OutsourcedServicesData) => void;
  isSubmitting: boolean;
  paymentMethods: PaymentMethod[];
}

const AddOutsourcedServiceModal: React.FC<AddOutsourcedServiceModalProps> = ({
  isOpen,
  onClose,
  onSave,
  isSubmitting,
  ...
  const [outsourcedServiceData, setOutsourcedServiceData] = useState<OutsourcedServicesData>({
    fecha: null,
    concepto: '',
    metodoPago: '',
    total: 0,
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = outsourcedServiceData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    await onSave(outsourcedServiceData);
  };
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="fecha"
                      value={outsourcedServiceData.fecha ? outsourcedServiceData.fecha.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
                  ...
                  <div>
                    ...
                    <select
                      name="concepto"
                      value={outsourcedServiceData.concepto}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      name="metodoPago"
                      value={outsourcedServiceData.metodoPago}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    >
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="total"
                      value={outsourcedServiceData.formattedTotal || ''}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium"
                      required
                    />
                ...
                {outsourcedServiceData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {outsourcedServiceData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <textarea
                  name="observaciones"
                  value={outsourcedServiceData.observaciones}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2.5 text-sm bg-white border border-gray-400 rounded focus:outline-none focus:border-[#3a6ea5] font-medium resize-none"
                  placeholder="INGRESE LAS OBSERVACIONES"
...
export default function OutsourcedServicesPage({ params }: PageProps) {
  ...
  // Datos
  const [registros, setRegistros] = useState<RegistroOutsourcedServices[]>([]);
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroOutsourcedServices | null,
    isDeleting: false
  });
  ...
  const [addOutsourcedServiceModal, setAddOutsourcedServiceModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroOutsourcedServices[]) => {
    const grouped: { [key: string]: RegistroOutsourcedServices[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/outsourced-services?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();

        setRegistros(data.map((registro: any) => ({
          id: registro.OutsourcedServiceID,
          fecha: registro.Date,
  ...
  const openDeleteModal = (registro: RegistroOutsourcedServices) => {
  ...
  const openEditModal = (registro: RegistroOutsourcedServices) => {
  ...
  const openAddOutsourcedServiceModal = () => {
    setAddOutsourcedServiceModal({ isOpen: true });
  };

  const closeAddOutsourcedServiceModal = () => {
    setAddOutsourcedServiceModal({ isOpen: false });
  };

  const handleAddOutsourcedService = async (data: OutsourcedServicesData) => {
    if (!projectId) {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/outsourced-services', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newOutsourcedServices = await response.json();
        const outsourcedServicesId = newOutsourcedServices.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);

          await fetch(`/api/project-manager/projects/outsourced-services?id=${outsourcedServicesId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              date: fechaFormateada,
        ...
        closeAddOutsourcedServiceModal();
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/outsourced-services?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroOutsourcedServices) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/outsourced-services?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar servicio subcontratado */}
      <AddOutsourcedServiceModal
        isOpen={addOutsourcedServiceModal.isOpen}
        onClose={closeAddOutsourcedServiceModal}
        onSave={handleAddOutsourcedService}
      ...
      <main className="pt-[72px] pb-[80px] min-h-screen bg-gray-100">
        <div className="w-full px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button
              onClick={openAddOutsourcedServiceModal}
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm
                      ...
                      : 'No se han encontrado registros de servicios subcontratados.'}
                  </p>
                </div>
              )}
            </div>
