      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full animate-fade-in relative z-[10000]">
        <div className="p-6 pb-4 border-b border-gray-300">
          ...
          <p className="text-sm text-gray-600 mt-2 leading-5">
            ¿Está seguro que desea eliminar el registro del vehículo <span className="font-bold">{registro?.vehiculo}</span>?
          </p>
...
interface EditModalProps {
  ...
  onClose: () => void;
  registro: RegistroCombustible | null;
  onSave: (data: RegistroCombustible) => void;
...
const EditModal: React.FC<EditModalProps> = ({
  ...
  const [editData, setEditData] = useState<RegistroCombustible | null>(null);
  ...
  const { startUpload } = useUploadThing('fuelFiles', {
  ...
  useEffect(() => {
    if (registro) {
      setEditData({
        ...registro,
        formattedTotal: registro.total.toLocaleString('en-US', {
          minimumFractionDigits: 2,
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'litros' || name === 'costoPorLitro') {
      const numericValue = parseFloat(value) || 0;

      setEditData(prev => {
        if (!prev) return null;

        const litros = name === 'litros' ? numericValue : prev.litros;
        const costoPorLitro = name === 'costoPorLitro' ? numericValue : prev.costoPorLitro;
        const totalTruncado = calculateTotal(litros, costoPorLitro);

        return {
          ...prev,
          [name]: numericValue,
          total: totalTruncado,
          formattedTotal: totalTruncado.toLocaleString('en-US', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          })
        };
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">EDITAR REGISTRO DE COMBUSTIBLE</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="SUV pasajeros">SUV PASAJEROS</option>
...
const AddFuelModal: React.FC<AddFuelModalProps> = ({
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = fuelData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
    ...
    <>
      <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 bg-black/70">
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              <h2 className="text-lg font-bold text-gray-900 tracking-tight">NUEVO REGISTRO DE COMBUSTIBLE</h2>
              <p className="text-gray-600 mt-1 text-sm">Complete la información del combustible de transportación local.</p>
            </div>
            <button
              onClick={handleClose}
              className="p-2 hover:bg-gray-100 rounded-full transition-colors"
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                {fuelData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {fuelData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
...
export default function LocalTransportationFuelPage({ params }: PageProps) {
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroCombustible | null,
    isDeleting: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroCombustible[]) => {
    const grouped: { [key: string]: RegistroCombustible[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/local-transportation-fuel?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();
        ...
        setRegistros(data.map((registro: any) => ({
          ...
          litros: parseFloat(registro.Liters) || 0,
          costoPorLitro: parseFloat(registro.LiterCost) || 0,
  ...
  const openDeleteModal = (registro: RegistroCombustible) => {
  ...
  const openEditModal = (registro: RegistroCombustible) => {
  ...
  const handleAddFuel = async (data: FuelData) => {
    ...
    try {
      ...
      if (isNaN(fechaObj.getTime())) {
        showModal('Error', 'Fecha inválida', 'error');
        return;
      }
      ...
      const response = await fetch('/api/project-manager/projects/local-transportation-fuel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        ...
        showModal('Éxito', '¡REGISTRO DE COMBUSTIBLE GUARDADO EXITOSAMENTE!', 'success');
        closeAddFuelModal();
      } else {
  ...
  const sendNotification = async (fuelId: number, isUpdate: boolean = false) => {
    try {
      await fetch('/api/notifications/notifications-fuel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/local-transportation-fuel?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroCombustible) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/local-transportation-fuel?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  const filteredRegistros = registros.filter(registro =>
    registro.vehiculo.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar combustible */}
      <AddFuelModal
        isOpen={addFuelModal.isOpen}
        onClose={closeAddFuelModal}
          ...
          <div className="mb-6">
            <div className="bg-[#3a6ea5] p-4 rounded-lg shadow border border-[#3a6ea5]">
              <h1 className="text-xl font-bold text-white tracking-tight flex items-center">
                ...
                GESTIÓN DE COMBUSTIBLES - TRANSPORTACIÓN LOCAL
              </h1>
              <p className="text-sm text-gray-200 mt-1">
                Administre y visualice todos los registros de combustible del proyecto.
              </p>
              {projectId && (
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Buscar por vehículo, observaciones..."
                  value={searchTerm}
                      ...
                      <table className="w-full">
                        <thead className="bg-gray-100">
                          <tr>
                            ...
                            <th className="py-3 px-4 text-left text-sm font-bold text-gray-700 uppercase border-b border-gray-300">VEHÍCULO</th>
                          ...
                          {weekRegistros.map((registro) => {
                            ...
                            return (
                              <tr key={registro.id} className="hover:bg-gray-50 transition-colors border-b border-gray-300">
                                ...
                                <td className="py-3 px-4">
                                  <div className="text-sm font-medium text-gray-800 uppercase max-w-[120px] truncate" title={registro.vehiculo}>
                                    {registro.vehiculo}
                                  </div>
                                </td>
                                <td className="py-3 px-4 text-sm text-gray-800 text-center font-medium">
                                  {registro.litros.toLocaleString('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                  })}
                                </td>
                                <td className="py-3 px-4 text-sm text-gray-800 font-medium">
                                  {formatCurrencyNumber(registro.costoPorLitro)}
                                </td>
                                <td className="py-3 px-4 text-sm text-gray-800 font-medium">
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm
                      ...
                      : 'No se han encontrado registros de combustibles.'}
                  </p>
                </div>
              )}
            </div>
