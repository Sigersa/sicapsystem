      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full animate-fade-in relative z-[10000]">
        <div className="p-6 pb-4 border-b border-gray-300">
          ...
          <p className="text-sm text-gray-600 mt-2 leading-5">
            ¿Está seguro que desea eliminar el registro del <span className="font-bold">{registro?.fecha ? formatDate(registro.fecha) : 'fecha desconocida'}</span>?
          </p>
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'litros' || name === 'costoPorLitro') {
      ...
      setEditData(prev => {
        ...
        const total = calculateTotal(litros, costoPorLitro);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      step="0.01"
                      name="total"
                      value={editData.total.toFixed(2)}
                      onChange={handleInputChange}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      step="0.01"
                      name="total"
                      value={fuelData.total.toFixed(2) || ''}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2.5 text-sm bg-gray-100 border border-gray-400 rounded font-medium text-gray-600"
                      readOnly
                    />
...
export default function LocalTransportationFuelClientPage({ params }: PageProps) {
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/fuel-client?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();

        setRegistros(data.map((registro: any) => ({
          id: registro.FuelClientID,
          fecha: registro.Date,
  ...
  const handleAddFuel = async (data: FuelData) => {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/fuel-client', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/fuel-client?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroCombustible) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/fuel-client?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Buscar por fecha, observaciones..."
                  value={searchTerm}
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm
                      ...
                      : 'No se han encontrado registros de combustible (cliente).'}
                  </p>
                </div>
              )}
            </div>
