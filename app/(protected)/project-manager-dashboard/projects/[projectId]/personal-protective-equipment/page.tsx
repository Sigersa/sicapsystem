...
const EditModal: React.FC<EditModalProps> = ({ 
  ...
  const [editData, setEditData] = useState<RegistroEpp | null>(null);
  ...
  const { startUpload } = useUploadThing('eppFiles', {
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-fade-in relative z-[10000]">
          <div className="p-6 pb-4 border-b border-gray-300 flex items-center justify-between sticky top-0 bg-white z-10">
            <div>
              ...
              <p className="text-gray-600 mt-1 text-sm">Modifique la información del registro de EPP.</p>
            </div>
            <button
              onClick={handleClose}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="Barbiquejos">Barbiquejos</option>
                      <option value="Cascos">Cascos</option>
                      <option value="Gafas">Gafas</option>
                      <option value="Guantes de Anticorte">Guantes de Anticorte</option>
                      <option value="Guantes de Impacto">Guantes de Impacto</option>
                      <option value="Guantes de Nitrilo">Guantes de Nitrilo</option>
                      <option value="Guantes de Piel">Guantes de Piel</option>
...
// ============ MODAL DE AGREGAR EPP ============

interface AddEppModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const AddEppModal: React.FC<AddEppModalProps> = ({ 
  isOpen, 
  onClose, 
  onSave, 
  isSubmitting, 
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const totalFiles = eppData.archivos.length + filesArray.length;
      if (totalFiles > 3) {
        setModal({
          isOpen: true,
          title: 'Error en archivos',
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      name="fecha"
                      value={eppData.fecha ? eppData.fecha.toISOString().split('T')[0] : ''}
                      onChange={(e) => {
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                {eppData.archivos.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {eppData.archivos.map((file, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                        <div className="flex items-center truncate">
                          <FileIcon type={file.type} size={5} />
                          <span className="ml-3 text-sm truncate font-medium text-gray-700">{file.name}</span>
...
export default function EppPage({ params }: PageProps) {
  ...
  const [deleteModal, setDeleteModal] = useState({
    isOpen: false,
    registro: null as RegistroEpp | null,
    isDeleting: false
  });
  ...
  const [addEppModal, setAddEppModal] = useState({
    isOpen: false
  });
  ...
  const groupRegistrosByWeek = (registros: RegistroEpp[]) => {
    const grouped: { [key: string]: RegistroEpp[] } = {};
  ...
  const fetchRegistros = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/epp?projectId=${projectId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const openDeleteModal = (registro: RegistroEpp) => {
  ...
  const openEditModal = (registro: RegistroEpp) => {
  ...
  const openAddEppModal = () => {
    setAddEppModal({ isOpen: true });
  };

  const closeAddEppModal = () => {
    setAddEppModal({ isOpen: false });
  };

  const handleAddEpp = async (data: EppData) => {
    if (!projectId) {
    ...
    try {
      ...
      const response = await fetch('/api/project-manager/projects/epp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: projectId,
      ...
      if (response.ok) {
        const newEpp = await response.json();
        const eppId = newEpp.id;

        let archivosSubidos: ArchivoAdjunto[] = [];
        if (data.archivos.length > 0) {
          archivosSubidos = await uploadFiles(data.archivos);
          
          await fetch(`/api/project-manager/projects/epp?id=${eppId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              date: fechaFormateada,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/epp?id=${deleteModal.registro.id}`, {
        method: 'DELETE',
      });
  ...
  const handleUpdate = async (data: RegistroEpp) => {
    ...
    try {
      ...
      const response = await fetch(`/api/project-manager/projects/epp?id=${data.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: fechaFormateada,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Modal de agregar EPP */}
      <AddEppModal
        isOpen={addEppModal.isOpen}
        onClose={closeAddEppModal}
        onSave={handleAddEpp}
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm 
                      ...
                      : 'No se han encontrado registros de EPP.'}
                  </p>
                </div>
              )}
            </div>
