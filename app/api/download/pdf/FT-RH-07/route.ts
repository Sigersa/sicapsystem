...
export async function GET(request: NextRequest) {
  ...
  const tempWordPath = path.join(
    os.tmpdir(),
    `FT-RH-07-${Date.now()}-${empleadoId}.docx`
  );
  const tempPdfPath = path.join(
    os.tmpdir(),
    `FT-RH-07-${Date.now()}-${empleadoId}.pdf`
  );
      ...
      try {
        ...
        if (pdfResponse.ok) {
          ...
          const fileName = employee.EmployeeType === 'PROJECT'
            ? `FT-RH-07-PROYECTO-${empleadoId}.pdf`
            : `FT-RH-07-BASE-${empleadoId}.pdf`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pdfResponse = await fetch(
      `${request.nextUrl.origin}/api/download/pdf/FT-RH-07?empleadoId=${empleadoId}`
    );
    ...
    return NextResponse.json({
      ...
      fileName: `FT-RH-07-${empleadoId}.pdf`
    });
