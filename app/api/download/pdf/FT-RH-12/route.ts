// app/api/download/pdf/FT-RH-12/route.ts
...
export async function GET(request: NextRequest) {
  ...
  console.log(`FT-RH-12: empleadoId=${empleadoId}, isPreview=${isPreview}, saveToDb=${saveToDb}`);
  ...
  if (isPreview && !saveToDb) {
    ...
    try {
      ...
      const [rows] = await connection.execute(
        'SELECT CDFileURL FROM jobtermination WHERE EmployeeID = ?',
        [parseInt(empleadoId)]
      );
      
      if (rows && (rows as any[]).length > 0 && (rows as any[])[0].CDFileURL) {
        const fileUrl = (rows as any[])[0].CDFileURL;
        console.log(`FT-RH-12 Vista previa: redirigiendo a ${fileUrl}`);
  ...
  const tempWordPath = path.join(
    os.tmpdir(),
    `FT-RH-12-${Date.now()}-${empleadoId}.docx`
  );
  const tempPdfPath = path.join(
    os.tmpdir(),
    `FT-RH-12-${Date.now()}-${empleadoId}.pdf`
  );
  ...
  try {
    ...
    if (!fs.existsSync(templatePath)) {
      return NextResponse.json(
        { error: "Plantilla FT-RH-12.docx no encontrada" },
        { status: 404 }
      );
    }
    ...
    if (saveToDb) {
      console.log(`FT-RH-12: saveToDb=true, subiendo a UploadThing...`);
      
      let fileUrl = null;
      try {
        const fileName = `FT-RH-12-${empleadoId}-${Date.now()}.pdf`;
        ...
        if (fileUrl) {
          ...
          try {
            await updateConnection.execute(
              `UPDATE jobtermination SET CDFileURL = ? WHERE EmployeeID = ?`,
    ...
    if (isPreview) {
      const fileName = employee.EmployeeType === 'PROJECT'
        ? `FT-RH-12-PROYECTO-${empleadoId}.pdf`
        : `FT-RH-12-BASE-${empleadoId}.pdf`;
    ...
    console.error("Error al generar FT-RH-12 PDF:", error);
    return NextResponse.json(
