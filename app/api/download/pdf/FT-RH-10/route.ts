// app/api/download/pdf/FT-RH-09/route.ts
...
export async function GET(request: NextRequest) {
  ...
  if (!movementId) {
    return NextResponse.json(
      { error: "Se requiere el ID del movimiento" },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const fileName = `FT-RH-10-${tipoEmpleado}-${mov.EmployeeID}.pdf`;
    ...
    console.error("Error al generar FT-RH-10 PDF:", error);
    return NextResponse.json(
      { 
        success: false,
