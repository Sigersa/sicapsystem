...
export async function GET(request: NextRequest) {
  ...
  const tempExcelPath = path.join(
    os.tmpdir(),
    `FT-RH-21-${Date.now()}-${empleadoId}.xlsx`
  );
  const tempPdfPath = path.join(
    os.tmpdir(),
    `FT-RH-21-${Date.now()}-${empleadoId}.pdf`
  );
  ...
  try {
    ...
    // Si ya existe un LoanFileURL y no estamos forzando la regeneración, usarlo
    if (employee.LoanFileURL && !saveToUploadThing) {
      pdfUrl = employee.LoanFileURL;
      try {
        ...
        if (pdfResponse.ok) {
          ...
          const fileName = employee.EmployeeType === 'PROJECT'
            ? `FT-RH-21-PROYECTO-${empleadoId}.pdf`
            : `FT-RH-21-BASE-${empleadoId}.pdf`;
    ...
    if (saveToUploadThing) {
      try {
        const fileName = `FT-RH-21-${fullName.replace(/\s+/g, '_')}-${Date.now()}.pdf`;
        ...
        if (uploadResult && uploadResult.data && uploadResult.data.url) {
          ...
          // Actualizar el campo LoanFileURL en la tabla employeeloans
          await connection.execute(
            `UPDATE employeeloans SET LoanFileURL = ? WHERE EmployeeID = ? ORDER BY LoanID DESC LIMIT 1`,
            [pdfUrl, empleadoId]
          );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pdfResponse = await fetch(
      `${request.nextUrl.origin}/api/download/pdf/FT-RH-21?empleadoId=${empleadoId}&saveUploadThing=1`
    );
    ...
    return NextResponse.json({
      ...
      fileName: `FT-RH-21-${empleadoId}.pdf`,
    });

  } catch (error: any) {
    console.error("Error en POST FT-RH-21:", error);
    return NextResponse.json(
